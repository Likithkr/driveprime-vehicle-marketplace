...
router.get('/', requireAdmin, async (req, res) => {
    try {
        ...
        const params = [];
        if (dealershipId) {
            sql += ' WHERE from_dealership_id = ? OR to_dealership_id = ? OR to_dealership_id IS NULL';
...
router.get('/unread-count', requireAdmin, async (req, res) => {
    try {
        const [rows] = await db.query('SELECT COUNT(*) as cnt FROM dealership_messages WHERE is_read = 0');
...
router.post('/', requireAdmin, async (req, res) => {
    try {
        ...
        const [rows] = await db.query('SELECT * FROM dealership_messages WHERE id = ?', [id]);
...
router.patch('/:id/read', requireAdmin, async (req, res) => {
    try {
        await db.query('UPDATE dealership_messages SET is_read = 1 WHERE id = ?', [req.params.id]);
        broadcast('messages:changed');
...
router.patch('/read-all', requireAdmin, async (req, res) => {
    try {
        await db.query('UPDATE dealership_messages SET is_read = 1 WHERE is_read = 0');
        broadcast('messages:changed');
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
router.delete('/:id', requireAdmin, async (req, res) => {
    try {
        await db.query('DELETE FROM dealership_messages WHERE id = ?', [req.params.id]);
        broadcast('messages:changed');
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
