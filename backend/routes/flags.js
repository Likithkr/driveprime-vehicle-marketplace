...
router.put('/:key', requireDeveloper, async (req, res) => {
    ...
    try {
        await db.query(
            'UPDATE feature_flags SET value = $1, updated_by = $2, updated_at = NOW() WHERE "key" = $3',
            [Boolean(value), req.user.email, req.params.key]
        );
        const [rows] = await db.query('SELECT * FROM feature_flags WHERE "key" = $1', [req.params.key]);
        res.json(rows[0]);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
