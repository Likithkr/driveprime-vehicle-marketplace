...
router.get('/me', require('../middleware/auth').requireAuth, async (req, res) => {
    const [rows] = await db.query('SELECT id, name, email, role, phone, created_at FROM users WHERE id = $1', [req.user.id]);
...
router.put('/profile', require('../middleware/auth').requireAuth, async (req, res) => {
    ...
    try {
        await db.query('UPDATE users SET name = $1, phone = $2 WHERE id = $3', [name.trim(), phone || '', req.user.id]);
        const [rows] = await db.query('SELECT id, name, email, role, phone, created_at FROM users WHERE id = $1', [req.user.id]);
        if (!rows.length) return res.status(404).json({ error: 'User not found' });
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        const [rows] = await db.query('SELECT id, name, role FROM users WHERE email = $1', [email.toLowerCase().trim()]);
...
router.post('/reset-password-otp', async (req, res) => {
    ...
    try {
        const [rows] = await db.query('SELECT id, reset_otp, reset_otp_expiry FROM users WHERE email = $1', [email.toLowerCase().trim()]);
        if (!rows.length) return res.status(404).json({ error: 'User not found' });
        const user = rows[0];
        ...
        const newHash = await bcrypt.hash(newPassword, 10);
        await db.query('UPDATE users SET password_hash = $1, reset_otp = NULL, reset_otp_expiry = NULL WHERE id = $2', [newHash, user.id]);
