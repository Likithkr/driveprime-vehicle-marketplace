...
export default function CarDetailPage() {
    ...
    // Open the Navbar customer auth modal via a custom event
    const openAuthModal = () => window.dispatchEvent(new CustomEvent('dp:open-auth'));

    // Drive Prime platform contact — used instead of seller's personal details
    const platformPhone = state.settings?.dealer_phone || '';
    const platformEmail = state.settings?.dealer_email || '';
    const platformWhatsApp = state.settings?.dealer_whatsapp || platformPhone;
                            ...
                            <div style={{
                                ...
                                {listing.status === 'live' && (
                                    <button
                                        ...
                                        style={{ width: '100%', justifyContent: 'center', marginTop: 12, fontSize: '0.95rem' }}
                                    >
                                        📅 Book a Viewing
                                    </button>
                                )}
